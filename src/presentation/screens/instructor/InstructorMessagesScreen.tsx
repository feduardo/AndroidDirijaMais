...
import { BookingAPIResponse } from '@/domain/entities/BookingAPI.types';
...
export const InstructorMessagesScreen = () => {
  ...
  const getAvatarColor = (name: string): string => {
    ...
    const index = name.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0) % colorPalette.length;
    return colorPalette[index];
  };

  const load = useCallback(async () => {
    try {
      const bookings: BookingAPIResponse[] = await InstructorRepository.listBookings(undefined);
      ...
      const chats: ChatItem[] = bookings.map((b) => {
        ...
        const studentName = b.student_name || 'Aluno';
  ...
  const renderItem = ({ item }: { item: ChatItem }) => {
    ...
    return (
      <TouchableOpacity
        activeOpacity={0.7}
        onPress={() => navigation.navigate('InstructorChat', { bookingId: item.bookingId })}
      >
  ...
  const renderHeader = () => (
    <View style={styles.header}>
      <Text style={styles.headerTitle}>Conversas com Alunos</Text>
